...
class _DatePickerDialogState extends State<DatePickerDialog>
  ...
  void _changeIsToDate(bool value) {
    setState(() {
      isToDate = value;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final Widget actions = Container(
        ...
        padding: const EdgeInsets.symmetric(
            ...
            vertical: MarginKeys.commonTopMargin),
        child: Row(
          children: [
            Padding(
              padding: const EdgeInsets.only(right: 10),
              child: Image.asset(Images.dateIcon),
            ),
            Text(isToDate ? StringKeys.noDate : todayDate),
            const Spacer(),
            TextButton(
    ...
    final Widget header = _DatePickerHeader(
      ...
      handleDateChangeCallback: _handleDateChanged,
      changeIsToDate: _changeIsToDate,
    );
...
/// non-null.
class _DatePickerHeader extends StatefulWidget {
  ...
  final bool isToDate;
  //current selected date on calender
  final DateTime? selectedDate;
  //callback funtion to handle custom date change from header
  final Function? handleDateChangeCallback;

  final Function? changeIsToDate;
  @override
  State<_DatePickerHeader> createState() => _DatePickerHeaderState();
}

class _DatePickerHeaderState extends State<_DatePickerHeader> {
  bool _isToday = true;
  bool _isNextMonday = false;
  bool _isNextTuesday = false;
  ...
  Widget build(BuildContext context) {
    ...
    final Text title = Text(
      widget.titleText,
      semanticsLabel: widget.titleSemanticsLabel ?? widget.titleText,
      style: widget.titleStyle,
      maxLines: widget.orientation == Orientation.portrait ? 1 : 2,
      overflow: TextOverflow.ellipsis,
    );
    ...
    switch (widget.orientation) {
      case Orientation.portrait:
        return SizedBox(
          height: !widget.isToDate
              ? _DatePickerHeader._datePickerHeaderPortraitHeight
              : 80,
          child: Material(
            color: EMPColors.white, //primarySurfaceColor,
            child: Padding(
              padding: const EdgeInsets.symmetric(
                horizontal: MarginKeys.margin16,
              ),
      ...
      case Orientation.landscape:
        return SizedBox(
          width: _DatePickerHeader._datePickerHeaderLandscapeWidth,
          child: Material(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                Padding(
                  padding: const EdgeInsets.symmetric(
                    horizontal: _DatePickerHeader._headerPaddingLandscape,
                  ),
                  child: help,
                ),
                SizedBox(height: widget.isShort ? 16 : 56),
                Expanded(
                  child: Padding(
                    padding: const EdgeInsets.symmetric(
                      horizontal: _DatePickerHeader._headerPaddingLandscape,
                ...
                if (widget.entryModeButton != null)
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 4),
                    child: widget.entryModeButton,
                  ),
              ],
            ),
          ),
  ...
  _buildFirstHeader(context) {
    return Row(
      children: [
        Expanded(
          child: TextButton(
            onPressed: _onTapToday,
            style: OutlinedButton.styleFrom(
              backgroundColor: EMPColors.fromHex(
                hexString: _isToday
                    ? EMPColors.lightBlueColor
                    : EMPColors.cancelBtnColor,
              ),
            ),
            child: Text(
              widget.isToDate ? StringKeys.noDate : StringKeys.today,
              style: TextStyles.titleText.copyWith(
                fontSize: 14,
                color: EMPColors.fromHex(
                  hexString: _isToday
                      ? EMPColors.hexWhiteColor
                      : EMPColors.lightBlueColor,
                ),
              ),
            ),
        ...
        Expanded(
          child: TextButton(
            onPressed: _onNextMondayTap,
            style: OutlinedButton.styleFrom(
              backgroundColor: EMPColors.fromHex(
                hexString: _isNextMonday
                    ? EMPColors.lightBlueColor
                    : EMPColors.cancelBtnColor,
              ),
            ),
            child: Text(
              widget.isToDate ? StringKeys.today : StringKeys.nextMonday,
              style: TextStyles.titleText.copyWith(
                fontSize: 14,
                color: EMPColors.fromHex(
                  hexString: _isNextMonday
  ...
  _buildSecondHeader() {
    return !widget.isToDate
        ? Row(
            children: [
              Expanded(
                child: TextButton(
                  onPressed: _onNextTuesdayTap,
                  style: OutlinedButton.styleFrom(
                    backgroundColor: EMPColors.fromHex(
                      hexString: _isNextTuesday
                          ? EMPColors.lightBlueColor
                          : EMPColors.cancelBtnColor,
                    ),
                  ),
                  child: Text(
                    StringKeys.nextTuesday,
                    style: TextStyles.titleText.copyWith(
                      fontSize: 14,
                      color: EMPColors.fromHex(
                        hexString: _isNextTuesday
              ...
              Expanded(
                child: TextButton(
                  onPressed: _onAfterWeekTap,
                  style: OutlinedButton.styleFrom(
                    backgroundColor: EMPColors.fromHex(
                      hexString: _isNextWeek
                          ? EMPColors.lightBlueColor
                          : EMPColors.cancelBtnColor,
                    ),
                  ),
                  child: Text(
                    StringKeys.afterWeek,
                    style: TextStyles.titleText.copyWith(
                      fontSize: 14,
                      color: EMPColors.fromHex(
                        hexString: _isNextWeek
...
//To handle today on click event
  _onTapToday() {
    widget.isToDate
        ? setState(() {
            _isToday = true;
  ...
  _onNextTuesdayTap() {
    setState(() {
      ...
      _isNextWeek = false;
      widget.handleDateChangeCallback!(
          widget.selectedDate?.nextDay(DateTime.tuesday));
    });
  }

  //method to after next week
  _onAfterWeekTap() {
    setState(() {
      _isToday = false;
      _isNextMonday = false;
      ...
      _isNextWeek = true;
      widget.handleDateChangeCallback!(
          widget.selectedDate?.nextDay(widget.selectedDate?.weekday));
    });
  }
}

//Custom extension method to get next date
extension DateTimeExtension on DateTime {
  DateTime nextDay(day) {
    int nextUpComingDay = 0;
    //calculate the next day
    if (day == weekday) {
      nextUpComingDay = 7;
    } else {
      nextUpComingDay = (day - weekday) % DateTime.daysPerWeek;
    }
//check for the months and fix the day
    int daysInMonth = DateUtils.getDaysInMonth(year, month);
    if (nextUpComingDay % daysInMonth != 0) {
      nextUpComingDay = nextUpComingDay % daysInMonth;
    }

    return add(Duration(days: nextUpComingDay));
  }
}
...
class _InputDateRangePickerDialog extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    log('selected ${selectedStartDate.toString()}');
    log('selectedEndDate ${selectedEndDate.toString()}');
